...
class TourRepository{
    ...
    GetTours() {
        ...
        for (const item of items) {
            tours.push(new Tour(item.id, item.name, item.country, item.price, item.maxTouristsCount, item.startDate));
        }
        return tours;
    }
    GetTourById(id) {
        ...
        for (const item of items) {
            if (item.id == id) {
                return new Tour(item.id, item.name, item.country, item.price, item.maxTouristsCount, item.startDate);
            }
        }
        return null;
    }
